...
void split_arcade()
{
    float left = axis3 + axis1;
    left = left * .12;
    float right = axis3 - axis1;
    right = right * .12;
    left_drive(left, 0, 0);
    right_drive(right, 0, 0);
}
...
float sweeper_out_pos = 100;
float sweeper_in = 240;
...
void sweeper_toggle()
{
    ...
    if (sweeper_out)
    {
        sweep.sweeper_pid_targ = sweeper_out_pos;
    }
    else
    {
        sweep.sweeper_pid_targ = sweeper_in;
    }
}
